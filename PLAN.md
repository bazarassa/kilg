...
todos:
  - id: backend-foundation
    content: Создать Go API, доменную модель, in-memory store и управление сессиями
    status: completed
  - id: reasoning
    content: Реализовать LLM прокси который сможет обеспечить выполнение тяжёлых запросов к reasoning-модели, которые могут выполняться 30 минут
    status: completed
  - id: workspace-api
    content: Написать код таким образом чтобы можно было авиоматически генерить open-api спеку с помощью swagger
    status: completed
  - id: verification-docs
    content: Добавить тесты, проверки сборки, инструкции запуска и вести общий ROADMAP.md
    status: completed
isProject: false
---
